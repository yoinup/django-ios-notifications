...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'APNService.private_key'
        db.delete_column('ios_notifications_apnservice', 'private_key')

        # Deleting field 'APNService.certificate'
        db.delete_column('ios_notifications_apnservice', 'certificate')

        # Deleting field 'APNService.passphrase'
        db.delete_column('ios_notifications_apnservice', 'passphrase')


    def backwards(self, orm):

        # User chose to not deal with backwards NULL issues for 'APNService.private_key'
        raise RuntimeError("Cannot reverse this migration. 'APNService.private_key' and its values cannot be restored.")

        # User chose to not deal with backwards NULL issues for 'APNService.certificate'
        raise RuntimeError("Cannot reverse this migration. 'APNService.certificate' and its values cannot be restored.")
        # Adding field 'APNService.passphrase'
        db.add_column('ios_notifications_apnservice', 'passphrase',
                      self.gf('django_fields.fields.EncryptedCharField')(max_length=101, null=True, cipher='AES', blank=True),
                      keep_default=False)
